...
inpath = '../output/your_shape_file.json'
shp_inpath = '../data/your_osm_extract.shp'
outpath = "../output/your_output_file.geojson"

turn_penalty_factor = 100 # Penalizes turns in Valhalla routes. Range 0 - 100,000.
stop_distance_threshold  = 500 # Stop-to-stop distance threshold for including intermediate coordinates (meters)
...
for count, segment in enumerate(segment_dict):
    ...
    #if count == 100: break # TEMP for debugging - to be deleted
...
shapefile = gpd.read_file(shp_inpath, crs='EPSG:4326')
...
for edge in edge_dict:
    ...
    sorted_points = [x for _,x in sorted(zip(distances, int_points))]
    ...
    for seg_index, segment in enumerate(bounds):
        ...
        if len(sorted_distances) == 0:
            ...
            start = full_line.project(Point(segment[0]))
            end = full_line.project(Point(segment[1]))
...
gdf['start_coord'] = start_list
gdf['end_coord'] = end_list
